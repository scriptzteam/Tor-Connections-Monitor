  ...
  with Controller.from_port(address=args.address, port=args.ctrlport) as controller:
    ...
    while True:
      ...
      try:
        ...
        connections = get_connections(resolver=args.resolver, process_pid=pid,
                        process_name='tor')
        t2 = time.time()
        ...
        for port in ports:
          ...
          stri = ' %5i %5i %6i %6i   %6i %6i %6i  (%s)' % (
            port, n_curr, n_opened, n_closed, MaxAll[port], MaxOpened[port],
            MaxClosed[port], port_usage(port))
...
if __name__ == '__main__':
  ...
  my_ipv4 = '1.1.1.1'
  my_ipv6 = ipaddress.IPv6Address('2606:4700:4700::1111').exploded
  main()
